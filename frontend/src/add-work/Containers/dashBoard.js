                ...
                <div className='widgetsContainer2'>
                    <div className='widgetsContainer5'>
                        <div className={`widget2 ${this.state.isCardFliped2 ? 'flip' : ''}`} onClick={() => this.flipCard2()}>
                            <Widgets className={'wid2'} width={"22vw"} height={"40vh"} title={"NEWS"} backText={"Ce widget présente les une nouvelle de l'actualité des crypto-monnaies. En cliquant sur le lien, une page s'ouvre vers l'article. "} widgetType={"NEWS"} data={news} />
                        </div>
                        <div className={`widget3 ${this.state.isCardFliped3 ? 'flip' : ''}`} onClick={() => this.flipCard3()}>
                            <Widgets className={'wid3'} id={"wid3"} width={"22vw"} height={"40vh"} title={"FEAR & GREED"} backText={"Ce widget présente un indicateur d'émotion du marché. Il est basé sur la volatilité, des sondages, le momentom et aussi Google trends pour les mots bitcoin ou crypto."} widgetType={"FEAR"} data={fear} />
                        </div>
                    </div>
                    ...
                    <div className='widgetsContainer6'>
                        ...
                        <div className={`widget6 ${this.state.isCardFliped6 ? 'flip' : ''}`} onClick={() => this.flipCard6()}>
                            <Widgets className={'wid6'} width={"22vw"} height={"40vh"} title={"WHALE ALERT"}
                                backText={"Ce widget présente des messages indiquant les transactions importantes et intéressantes au fur et à mesure qu'elles se produisent. Il permet de savoir quand les gros portefeuilles font des transactions."} widgetType={"WHALE"} data={whale} />
                        </div>
                    </div>
                </div>
            </div>
